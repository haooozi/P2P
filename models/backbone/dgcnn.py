...
class DGCNN(nn.Module):
    ...
    def __init__(self,
        ...
        point_channel = 0
        for i in gf_channels:
            point_channel += i[0]
        self.FA_module = nn.Sequential(
            nn.Conv1d(point_channel, fa_channels, 1, bias=False),
            nn.SyncBatchNorm(fa_channels, eps=1e-3, momentum=0.01),
            nn.ReLU(True),
    ...
    def forward(self, points):
        ...
        cat_feats = torch.cat(gf_points[1:], dim=-1).transpose(1, 2).contiguous()
        glb_feats = self.FA_module(cat_feats)
        return glb_feats
