...
class RealNVP(nn.Module):
    ...
    def get_scale_net():
        ...
        return nn.Sequential(
            nn.Linear(3, 64), nn.LeakyReLU(), nn.Linear(64, 64),
            nn.LeakyReLU(), nn.Linear(64, 3), nn.Tanh())
    ...
    def __init__(self):
        ...
        self.register_buffer('loc', torch.zeros(3))
        self.register_buffer('cov', torch.eye(3))
        self.register_buffer(
            'mask', torch.tensor(
                [[1, 1, 0], [1, 0, 1], [0, 1, 1]] * 3, dtype=torch.float32))
...
class RLELoss(nn.Module):
    ...
    def forward(self, pred, sigma, target):
        ...
        log_sigma = torch.log(sigma).reshape(target.shape[0], 3)
